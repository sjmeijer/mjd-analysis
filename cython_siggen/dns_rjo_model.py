...
def get_velo_params():
    return (priors[velo_first_idx:velo_first_idx+6], velo_width)
def get_t0_params():
...
class Model(object):
    ...
    def perturb(self, params):
        ...
        """
        logH = 0.0
        self.changed_wfs[:] = False
        which = rng.randint(2)

        if which == 0:
            #perturb detector level params
            self.perturb_detector(params)
            self.changed_wfs[:] = True

        elif which == 1:
            #perturb wf level params
            count = 0
            rand_num = rng.randn()/np.sqrt( -np.log(rng.rand()));

            chance = np.power(10.0, -3*np.abs(rand_num))
            for i in range(num_waveforms):
                if rng.rand() <= chance:
            		self.changed_wfs[i] = True;
            		count+=1
            if(count == 0):
                self.changed_wfs[rng.randint(num_waveforms)] = True;
            for i in range(num_waveforms):
                if self.changed_wfs[i]:
                    self.perturb_wf(params, i)
        return logH
